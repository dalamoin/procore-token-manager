...
class TokenManager:
    """Production OAuth and token management service"""
    ...
    def __init__(self):
        ...
        self.client_id = self._get_secret_value('procore-client-id-production')
        self.client_secret = self._get_secret_value('procore-client-secret-production')
        self.redirect_uri = self._get_secret_value('procore-redirect-uri-production')
        ...
        if not self.client_id:
            logger.error(f"No client ID found for production environment")
        if not self.client_secret:
            logger.error(f"No client secret found for production environment")
        if not self.redirect_uri:
            logger.error(f"No redirect URI found for production environment")
        
        logger.info(f"TokenManager initialized: environment={self.environment}, project_id={self.project_id}")
                
        # Set OAuth base URL for production
    ...
    def _get_secret_names(self) -> Dict[str, str]:
        """Get production secret names"""
        return {
            'access_token': 'procore-access-token-production',
            'refresh_token': 'procore-refresh-token-production'
        }
    ...
    def exchange_code_for_tokens(self, authorization_code: str) -> Dict[str, any]:
        ...
        if not self.client_id or not self.client_secret:
            error_msg = f"Missing credentials for production: client_id={bool(self.client_id)}, client_secret={bool(self.client_secret)}"
        ...
        try:
            logger.info(f"Exchanging authorization code for tokens (production)")
            ...
            if response.status_code == 200:
                ...
                logger.info(f"OAuth successful - tokens stored in Secret Manager for production")
                return {
                    'success': True,
                    'message': 'OAuth tokens exchanged and stored successfully for production',
                    'environment': self.environment,
                    'timestamp': datetime.now(timezone.utc).isoformat()
                }
            else:
    ...
    def get_current_tokens(self) -> Dict[str, Optional[str]]:
        ...
        try:
            ...
            logger.error(f"Failed to get tokens from Secret Manager for production: {e}")
            return {'access_token': None, 'refresh_token': None}
    
    def refresh_access_token(self) -> Dict[str, any]:
        """Refresh access token using refresh token"""
        logger.info(f"Starting access token refresh process for production")
        tokens = self.get_current_tokens()
        refresh_token = tokens.get('refresh_token')
        
        if not refresh_token:
            error_msg = 'No refresh token available for production'
            logger.error(error_msg)
            return {
                'success': False,
                'error': error_msg,
        ...
        try:
            logger.info(f"Refreshing Procore access token for production")
            response = requests.post(url, data=data, headers=headers, timeout=30)
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Access token refresh successful for production")
                return {
                    'success': True,
                    'environment': self.environment,
                    'timestamp': datetime.now(timezone.utc).isoformat()
    ...
    def _store_tokens(self, access_token: str, refresh_token: str):
        ...
        secret_names = self._get_secret_names()
        logger.info(f"Storing tokens in Secret Manager for production")
        ...
        logger.info(f"Tokens stored and old versions destroyed for production")
...
def token_manager_handler(request):
    """HTTP Cloud Function handler for production token management"""
    logger.info("Production token manager handler started")
    ...
    try:
        ...
        logger.info(f"Creating TokenManager for production")
        ...
        if code:
            logger.info(f"Processing OAuth callback for production")
            result = token_manager.exchange_code_for_tokens(code)
            
            if result['success']:
                return f'''<html><body>
                    <h1 style="color: #dc3545">Authentication Successful! (PRODUCTION)</h1>
                    <p>Tokens have been stored in Secret Manager for <strong>production</strong> environment.</p>
        ...
        return f'''<html><head>
            <title>Procore Production Token Manager</title>
            <style>
                ...
                .auth-button {{ background-color: #dc3545; }}
                ...
                .disclaimer {{ background-color: #ffe5e5; padding: 15px; border-radius: 5px; margin: 10px 0; border-left: 4px solid #dc3545; }}
            </style>
            <script>
                async function refreshTokens() {{
                    ...
                    try {{
                        const response = await fetch('https://us-central1-serendia.cloudfunctions.net/production-token-refresh', {{
                            method: 'POST',
            ...
            <h1>Procore Production Token Manager</h1>
            
            <div class="disclaimer">
                <h4>⚠️ PRODUCTION ENVIRONMENT</h4>
                <p>This is the PRODUCTION token manager. Changes will affect live systems.</p>
            </div>
            
            <div class="info">
                <h3 style="color: #dc3545">Authentication</h3>
                <a href="{token_manager.get_oauth_url()}" class="button auth-button">
                    Authenticate with Procore (Production)
                </a>
            </div>
            ...
            <div class="disclaimer">
                ...
                <p>The <code>production-token-refresh</code> service automatically handles access token refresh <strong>every hour</strong>. 
